...
- The variance of $y$ (denoted as $\text{var}(y)$) tells us how much do Massachusetts high schools differ in average math SAT scores
- The variance of $\widehat{y}$ (denoted as $\text{var}(\widehat{y})$) tells us how much the fitted values from the interaction model vary
- The variance of the residuals tells us how much do "the left-overs" from the model vary
- Define $R^2$:
$$
...
- Repeat this $R^2$ comparison between interaction and parallel slopes model for our models of $y$ = teaching score for UT Austin professors:
...
- Since the additional complexity of the interaction model over the parallel slopes model improves our $R^2$ value by a relatively large amount, it could be argued that the additional complexity is warranted 
- We can also use `get_regression_summaries()` to quickly automate calculating $R^2$ for both the interaction and parallels slopes models for $y$ = average math SAT score for Massachusetts high schools
...
- The left-hand plot focuses on the relationship between `debt` and `income` in *aggregate*
- The right-hand plot focuses on the relationship between `debt` and `income` *broken down by `credit_limit` bracket*
  - Four *separate* relationships between `debt` and `income`: one for the "low" `credit_limit` bracket, one for the "medium-low" `credit_limit` bracket, and so on 
